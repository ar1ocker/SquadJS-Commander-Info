...
export default class CommandersInfo extends BasePlugin {
  static get description() {
    return (
        'Плагин выводящий броадкастом CMD сторон если они есть с двух сторон, выводит 1 раз через определенное время'
    );
  }
  ...
  async searchCommanders() {
    if (this.searchCommandersTimeout) clearTimeout(this.searchCommandersTimeout);
    ...
    if (commanders.length === 2 && commanders[0].name && commanders[1].name) {
      await this.server.rcon.broadcast(`CMD сторон: ${commanders[0].name} и ${commanders[1].name}!`);
      return;
    }

    this.searchCommandersTimeout = setTimeout(this.searchCommanders, this.options.repeatSearchTimeout);
  }
  ...
  async mount() {
    this.server.on('NEW_GAME', () => {
      this.count_searches = 0;
      if (this.searchCommandersTimeout) clearTimeout(this.searchCommandersTimeout);
      this.searchCommandersTimeout = setTimeout(this.searchCommanders, this.options.startSearchTimeout);
    })
  }
}
